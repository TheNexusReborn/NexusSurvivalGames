...
public class GameTimerResumeSubCommand extends SubCommand {
    
    private SurvivalGames plugin;
    
    public GameTimerResumeSubCommand(SurvivalGames plugin, NexusCommand parent) {
        super(parent, "resume", "Resume the timer", parent.getMinRank());
        this.plugin = plugin;
    }
    ...
    public void handleCommand(NexusCommand nexusCommand, CommandActor actor, String[] previousArgs, String label, String[] args) {
        ...
        game.sendMessage("&eThe timer has been resumed by &b" + actor.getPlayer().getName());
        if (!game.hasPlayer(actor.getPlayer().getUniqueId())) {
            actor.sendMessage("&eYou resumed the timer of the game.");
        }
    }
}
