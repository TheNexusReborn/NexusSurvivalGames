...
public class PlayerMenu extends InventoryGUI implements UpdatingGUI {
    ...
    public void createItems() {
        ...
        if (actorGamePlayer.getTeam() == GameTeam.SPECTATORS || actorGamePlayer.getRank().ordinal() <= Rank.ADMIN.ordinal()) {
            Button teleportButton = new Button().iconCreator(p -> ItemBuilders.of(SMaterial.ENDER_PEARL).displayName("&e&lTeleport").build())
                    .consumer(e -> {
            ...
            if (player.getTeam() != GameTeam.SPECTATORS) {
                ...
                Element vitalsElement = new Element().iconCreator(p -> ItemBuilders.of(SMaterial.GLASS_BOTTLE).displayName("&e&lVitals").setLore(vitalsLore).build());
                addElement(vitalsElement);

                Button viewInventoryButton = new Button().iconCreator(p -> ItemBuilders.of(SMaterial.ENDER_CHEST).displayName("Inventory").addLoreLine("&c&lNOT YET IMPLEMENTED").build())
            ...
            if (sponsoringAllowed && player.getToggleValue("allowsponsors") && player.getTeam() == GameTeam.TRIBUTES) {
                Button sponsorButton = new Button().iconCreator(p -> ItemBuilders.of(SMaterial.CHEST).displayName("&e&lSponsor").build())
                        .consumer(e -> {
                            SGPlayer actor = plugin.getPlayerRegistry().get(e.getWhoClicked().getUniqueId());
        ...
        Element statsElement = new Element().iconCreator(p -> ItemBuilders.of(SMaterial.PAPER).displayName("&e&lStats").setLore(statsLore).build());
        addElement(statsElement);

        Element typeElement = new Element().iconCreator(p -> ItemBuilders.of(SMaterial.PLAYER_HEAD).displayName(player.getTeam().getColor() + "&l" + player.getTeam().name().substring(0, player.getTeam().name().length() - 1)).build());
        ...
        Button manageButton = new Button().iconCreator(p -> ItemBuilders.of(SMaterial.REDSTONE_BLOCK).displayName("&c&lMANAGE").build()).consumer(e -> manager.openGUI(new PlayerManageMenu(plugin, actorPlayer.getGame(), new PlayerManageBuilder(actorPlayer, player)), e.getWhoClicked()));
