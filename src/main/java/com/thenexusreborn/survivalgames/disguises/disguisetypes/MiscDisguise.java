...
import com.thenexusreborn.survivalgames.disguises.disguisetypes.watchers.DroppedItemWatcher;
import com.thenexusreborn.survivalgames.disguises.disguisetypes.watchers.FallingBlockWatcher;
import com.thenexusreborn.survivalgames.disguises.disguisetypes.watchers.PaintingWatcher;
import com.thenexusreborn.survivalgames.disguises.disguisetypes.watchers.SplashPotionWatcher;
...
public class MiscDisguise extends TargetedDisguise {
        ...
        switch (disguiseType) {
            ...
            case FALLING_BLOCK ->
                    ((FallingBlockWatcher) getWatcher()).setBlock(new ItemStack(Math.max(1, firstParam), 1, (short) Math.max(0,
                            secondParam)));
            case SPLASH_POTION -> ((SplashPotionWatcher) getWatcher()).setPotionId(Math.max(0, firstParam));
            case DROPPED_ITEM -> {
                if (firstParam > 0) {
                    ((DroppedItemWatcher) getWatcher()).setItemStack(new ItemStack(firstParam, Math.max(0, secondParam)));
                }
            } // Entity ID of whoever is holding fishing rod
            // Entity ID of shooter. Used for "Is he on this scoreboard team and do I render it moving through his body?"
            // Unknown. Uses entity id of shooter. 0 if no shooter
            // Unknown. Uses entity id of shooter. 0 if no shooter
            case FISHING_HOOK, ARROW, SMALL_FIREBALL, FIREBALL, WITHER_SKULL -> // Unknown. Uses entity id of shooter. 0 if no shooter
    ...
    public int getData() {
        return switch (getType()) {
            case FALLING_BLOCK -> ((FallingBlockWatcher) getWatcher()).getBlock().getDurability();
            case PAINTING -> ((PaintingWatcher) getWatcher()).getArt().getId();
            case SPLASH_POTION -> ((SplashPotionWatcher) getWatcher()).getPotionId();
            default -> data;
        };
    }
