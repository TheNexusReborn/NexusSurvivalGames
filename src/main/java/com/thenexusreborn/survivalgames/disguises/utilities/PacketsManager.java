...
import com.thenexusreborn.survivalgames.disguises.disguisetypes.watchers.LivingWatcher;
import com.thenexusreborn.survivalgames.disguises.disguisetypes.watchers.PlayerWatcher;
import com.thenexusreborn.survivalgames.disguises.disguisetypes.watchers.SheepWatcher;
import com.thenexusreborn.survivalgames.disguises.disguisetypes.watchers.WolfWatcher;
import com.thenexusreborn.survivalgames.disguises.utilities.DisguiseSound.SoundType;
...
public class PacketsManager {
    ...
    private static NexusDisguises libsDisguises;
    private static PacketListener mainListener;
    private static PacketListener soundsListener;
    private static boolean soundsListenerEnabled = true;
    ...
    public static void addPacketListeners() {
        ...
        clientInteractEntityListener = new PacketAdapter(plugin, ListenerPriority.NORMAL,
    ...
    public static PacketContainer[][] constructSpawnPackets(final Player player, Disguise disguise, Entity disguisedEntity) {
        ...
        if (disguise.getType() == DisguiseType.EXPERIENCE_ORB) {
            ...
            spawnPackets = newPackets.toArray(new PacketContainer[0]);
    ...
    public static void init(NexusDisguises disguises) {
        ...
        viewDisguisesListener = new PacketAdapter(plugin, ListenerPriority.HIGH,
        ...
        inventoryListener = new PacketAdapter(plugin, ListenerPriority.HIGHEST, PacketType.Play.Server.SET_SLOT,
                        ...
                        // If the player switched item, aka he moved from slot 1 to slot 2
                            ...
                            if (event.getPacket().getIntegers().read(3) == 1) {
                                ...
                                if (clickedItem != null && clickedItem.getType() != Material.AIR) {
                                    ...
                                    Bukkit.getScheduler().runTask(plugin, () -> event.getPlayer().updateInventory());
                                }
                                return;
                            } else {
    ...
    public static void setupMainPacketsListener() {
        if (clientInteractEntityListener != null) {
            ...
            mainListener = new PacketAdapter(plugin, ListenerPriority.HIGH, packetsToListen) {
                @Override
                public void onPacketSending(PacketEvent event) {
                    if (event.isCancelled())
                        return;
                ...
                // Else if the disguise is moving.
                    ...
                    if (disguise.getType() == DisguiseType.RABBIT
                        ...
                        if (entity.getMetadata("LibsRabbitHop").isEmpty()
                            ...
                            if (entity.getMetadata("LibsRabbitHop").isEmpty()
                                    || System.currentTimeMillis() - entity.getMetadata("LibsRabbitHop").get(0).asLong() > 500) {
                                entity.removeMetadata("LibsRabbitHop", plugin);
