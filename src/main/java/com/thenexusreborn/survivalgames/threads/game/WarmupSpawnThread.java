...
public class WarmupSpawnThread extends NexusThread<SurvivalGames> {
    ...
    public static final Stream<GameState> stateStream = Stream.of(GameState.WARMUP, GameState.WARMUP_DONE, GameState.DEATHMATCH_WARMUP, GameState.DEATHMATCH_WARMUP_DONE, GameState.TELEPORT_START, GameState.TELEPORT_DEATHMATCH, GameState.TELEPORT_START_DONE, GameState.TELEPORT_DEATHMATCH_DONE);
    ...
    public void onRun() {
        ...
        if (stateStream.noneMatch(gameState -> game.getState() == gameState)) {
            return;
        }

        BiMap<UUID, Integer> spawns = game.getSpawns().inverse();
