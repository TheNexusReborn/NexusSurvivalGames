...
public class UnzipMapStep extends GamePhaseStep {
    public UnzipMapStep(GamePhase gamePhase, GamePhaseStep... prerequisiteSteps) {
        super(gamePhase, "unzip_map", prerequisiteSteps);
    }
    
    @Override
    public boolean run() {
        setStatus(StepStatus.STARTING);
        if (!game.getGameMap().unzip(Game.getPlugin())) {
